...
import Rules.BinaryRule;
import Rules.RuleMeasures;
...
public class GibbsSampling {
	...
	public GibbsSampling(int nbSamples, int[][] transactions, int nbItems) {
        this.transactions = transactions;
    ...
    public void sample(int nbIterations) {
        ...
        for (int itt = 1; itt < nbIterations; itt++) {
            int[] X = Arrays.copyOf(mockSample[itt - 1].getX(), mockSample[itt - 1].getX().length);  // Create a deep copy of J.getX()
        ...
        // Filter out rules with support 0 from the mock sample to create the real sample
    ...
    public String toString() {
        ...
        // Header
        sb.append(String.format("%-25s%-10s%-10s%n", "Rule", "Support", "Confidence"));
        ...
        for (BinaryRule rule : this.sample) {
            ...
            sb.append(String.format("%-25s%-10.4f%-10.4f%n", Arrays.toString(X) + " => " + Arrays.toString(Y), support, confidence));
        }
