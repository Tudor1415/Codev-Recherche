 ...
 * (see https://github.com/chris58/ahp/blob/master/src/main/java/com/chris/ahp/AHP.java)
...
public class AHP {
    ...
    /**
     * Compute weigths w.r.t. the AHP matrix A
     * @param A matrix
     */
    public void computeWeights(double[][] A) {
        int k = 0;
        for (int i = 0; i < A.length; i++) {
            for (int j = i + 1; j < A.length; j++) {
                pairwiseComparisonArray[k] = A[i][j];
                k++;
            }
        }
        setPairwiseComparisonArray(pairwiseComparisonArray);
    }
